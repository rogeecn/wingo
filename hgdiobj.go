...
func (hGdiObj HGDIOBJ) DeleteObject() {
	ret, _, err := syscall.Syscall(proc2.DeleteObject.Addr(), 1,
		uintptr(hGdiObj), 0, 0)
	if ret == 0 {
...
func CreateBitmap(width, height int32,
	...
	ret, _, err := syscall.Syscall6(proc2.CreateBitmap.Addr(), 5,
...
func CreateBitmapIndirect(bmp *BITMAP) HBITMAP {
	ret, _, err := syscall.Syscall(proc2.CreateBitmapIndirect.Addr(), 1,
...
func (hBmp HBITMAP) GetObject(bmp *BITMAP) {
	ret, _, err := syscall.Syscall(proc2.GetObject.Addr(), 3,
...
func CreateBrushIndirect(lb *LOGBRUSH) HBRUSH {
	ret, _, err := syscall.Syscall(proc2.CreateBrushIndirect.Addr(), 1,
...
func CreateHatchBrush(hatch co.HS, color COLORREF) HBRUSH {
	ret, _, err := syscall.Syscall(proc2.CreateHatchBrush.Addr(), 2,
...
func CreatePatternBrush(hBmp HBITMAP) HBRUSH {
	ret, _, err := syscall.Syscall(proc2.CreatePatternBrush.Addr(), 1,
		uintptr(hBmp), 0, 0)
	if ret == 0 {
		panic(errco.ERROR(err))
	}
...
func CreateSolidBrush(color COLORREF) HBRUSH {
	ret, _, err := syscall.Syscall(proc2.CreateSolidBrush.Addr(), 1,
		uintptr(color), 0, 0)
	if ret == 0 {
		panic(errco.ERROR(err))
	}
...
func GetSysColorBrush(index co.COLOR) HBRUSH {
	ret, _, err := syscall.Syscall(proc2.GetSysColorBrush.Addr(), 1,
		uintptr(index), 0, 0)
	if ret == 0 {
		panic(errco.ERROR(err))
	}
...
func CreateFontIndirect(lf *LOGFONT) HFONT {
	ret, _, err := syscall.Syscall(proc2.CreateFontIndirect.Addr(), 1,
...
func CreatePen(style co.PS, width int32, color COLORREF) HPEN {
	ret, _, err := syscall.Syscall(proc2.CreatePen.Addr(), 3,
...
func CreatePenIndirect(lp *LOGPEN) HPEN {
	ret, _, err := syscall.Syscall(proc2.CreatePenIndirect.Addr(), 1,
...
func CreateRectRgnIndirect(rc *RECT) HRGN {
	ret, _, err := syscall.Syscall(proc2.CreateRectRgnIndirect.Addr(), 1,
...
func CreateRoundRectRgn(x1, y1, x2, y2, w, h int32) HRGN {
	ret, _, err := syscall.Syscall6(proc2.CreateRoundRectRgn.Addr(), 6,
...
func (hRgn HRGN) CombineRgn(hrgnSrc1, hrgnSrc2 HRGN, mode co.RGN) co.REGION {
	ret, _, err := syscall.Syscall6(proc2.CombineRgn.Addr(), 4,
...
func (hRgn HRGN) OffsetRgn(x, y int32) co.REGION {
	ret, _, err := syscall.Syscall(proc2.OffsetRgn.Addr(), 3,
